...
`"…"`    	|           	|string-literal   	|String literal stored as an array of codepoints.  Unterminated string literals will be printed implicitly.  `` ` `` is the escape character for `` ` `` and `"`. The characters `0tnr` do the same as C when escaped.  `1` includes a popped value as a string template.  All other escaped single characters will execute as stax code and then include the popped value as a template.
