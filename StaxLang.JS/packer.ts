...
const _95 = int.make(95), _x100 = int.make(0x100), _x80 = int.make(0x80);
...
export function packBytes(asciiStax: string): number[] {
    ...
    for (let i = asciiStax.length - 1; i >= 0; i--) {
        big = int.add(int.mul(big, _95), int.make(asciiStax.charCodeAt(i) - 32));
    }
    while (int.floatify(big) > 0) {
        let b = int.mod(big, _x100);
        if (int.eq(big, b)) {
            if (int.eq(int.bitand(b, _x80), int.zero)) {
                b = int.bitor(b, _x80); // set leading bit for packing flag
            }
...
export function unpackBytes(bytes: number[] | Uint8Array): string {
    ...
    for (let i = 0; i < bytes.length; i++) {
        big = int.add(int.mul(big, _x100), int.make(bytes[i]));
    }
    while (int.floatify(big) > 0) {
        result += String.fromCharCode(int.floatify(int.mod(big, _95)) + 32);
        big = int.div(big, _95);
    }
